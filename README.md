...
[![CRAN
status](https://www.r-pkg.org/badges/version/s7contract)](https://CRAN.R-project.org/package=s7contract)
...
and shrinking draw on Hedgehog. `implements()` checks method
availability, `has_trait()` checks declared implementation, and
`check_law()` reports sampled evidence about behavior.
...
sampled case. `expect_law()` executes the cases, shrinks the first
counterexample, and returns one ordinary tinytest result.
...
area_law <- new_law(
  ...
  generators = list(
    x = gen_map(gen_double(0, 100), function(r) Circle(r = r))
  ),
...
Generators construct valid examples; S7 validators check the objects
they produce. `gen_bind()` builds dependent inputs, such as a length and
a vector of that length, and preserves the relationship during
shrinking. `gen_choice()` selects among generators, and
`gen_recursive()` builds nested values with decreasing recursive size.
See [Generative Laws with
tinytest](https://sounkou-bioinfo.github.io/s7contract/articles/property-laws.html)
for generator composition, shrinking, and replay.
