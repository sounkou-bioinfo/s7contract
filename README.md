...
The Rust-like layer adds an explicit `impl_trait()` registry on top of
...
- **Accurate for S7:** S7 generics define interfaces and methods belong
  ...
  here.
- **Accurate for this package:** `implements()` is structural. It checks
  whether `S7::method()` can find a method for every required generic.
- **Accurate for this package:** `has_trait()` is nominal. Existing S7
  methods are not enough; `impl_trait()` must have recorded the
  implementation.
