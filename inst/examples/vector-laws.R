...
vector_laws <- function(make, element = gen_double(-10, 10)) {
  ...
  list(
    values = new_law(
      "values preserve constructor input",
      list(input = cases),
      function(input) {
        with(VectorLike, {
    ...
    length = new_law(
      "length agrees with constructor input",
      list(input = cases),
      function(input) {
        with(VectorLike, {
    ...
    slice_values = new_law(
      "slicing preserves selected values and order",
      list(input = cases),
      function(input) {
        with(VectorLike, {
          identical(
            vec_values(vec_slice(input$x, input$i)),
      ...
      classify = function(input) {
        c(
          ...
          if (
            length(input$i) == length(input$values) && !anyDuplicated(input$i)
          ) {
            "permutation"
          },
    ...
    slice_length = new_law(
      "slice length matches the index count",
      list(input = cases),
      function(input) {
        with(VectorLike, {
          identical(
            vec_length(vec_slice(input$x, input$i)),
            base::length(input$i)
          )
        })
      }
...
implementations <- list(
  numeric = identity,
  read_depth = function(values) {
    ReadDepth(position = seq_along(values), depth = values)
  }
)
...
broken_results <- lapply(
  vector_laws(
    function(values) {
      ReversedDepth(position = seq_along(values), depth = values)
    },
...
empty_only <- check_law(
  vector_laws(identity)$slice_values,
  tests = 10L,
  seed = 1L,
  max_size = 0L
)
...
rounded <- function(values) {
  RoundedDepth(position = seq_along(values), depth = values)
}

integer_check <- check_law(
  vector_laws(rounded, whole_numbers)$values,
  seed = 1L
)
