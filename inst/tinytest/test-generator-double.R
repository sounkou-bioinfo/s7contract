...
for (bounds in list(c(-5, 12), c(2, 8), c(-8, -2))) {
  ...
  result <- check_law(new_law(
    "origin",
    list(x = gen_resize(generator, 100L)),
    function(x) FALSE
  ))
...
for (value in c(
  ...
  result <- check_law(new_law("constant", list(x = generator), function(x) {
    FALSE
  }))
...
for (seed in 1:12) {
  ...
  expect_identical(
    gen_example(generator, size = .Machine$integer.max, seed = seed),
    full
  )
}
...
for (bounds in list(c(1e292, 1e308), c(-1e308, -1e292))) {
  ...
  for (seed in 1:20) {
    expect_identical(
      gen_example(from_lower, size = 100L, seed = seed),
...
domains <- list(
  ...
  c(-huge, huge, huge),
  c(-huge, huge, -huge),
  c(huge / 2, huge, huge / 2),
  c(-huge, -huge / 2, -huge / 2),
  c(0, tiny, 0),
  c(-tiny, 0, 0),
...
for (domain in domains) {
  local({
    generator <- gen_resize(
      gen_double(domain[[1L]], domain[[2L]], domain[[3L]]),
      100L
    )
    ...
    if (original != domain[[3L]]) {
      ...
      expect_true(all(
        children >= min(original, domain[[3L]]) &
...
result <- check_law(
  new_law("zero", list(x = leaf), function(x) x == 0),
  tests = 1L,
  seed = 4L
)
...
local({
  calls <- 0L
  generator <- gen_map(
    gen_resize(gen_double(0, 8), 100L),
    function(x) {
      calls <<- calls + 1L
      x
    },
  ...
  expect_true(
    bounded@counterexample@minimal$x < bounded@counterexample@original$x
  )
})

# Dependent intervals and nested vectors retain bounds and atomic prototypes.
local({
  source <- new_generator(
    function(size) 4L,
    function(n) if (n > 1L) list(1L) else list(),
    prototype = integer()
  )
  generator <- gen_bind(source, function(n) {
    gen_product(
      n = gen_constant(n),
      x = gen_vector(
        gen_vector(gen_double(n, n + 0.5), min = 1L, max = 3L),
        min = 1L,
        max = 3L
      )
    )
  ...
  law <- new_law("nested doubles", list(input = generator), function(input) {
    ...
    for (x in input$x) {
      stopifnot(length(x) >= 1L, all(x >= input$n), all(x <= input$n + 0.5))
    }
    FALSE
  })
  ...
  expect_identical(result@shrink_status, "complete")
  expect_identical(
    result@counterexample@minimal$input,
    list(n = 1L, x = list(1))
  )
...
for (exception in c(NA_real_, NaN, -Inf, Inf)) {
  only_exception <- gen_choice(
    gen_double(),
    gen_constant(exception),
    prob = c(0, 1)
  )
  expect_identical(
    gen_example(gen_vector(only_exception, 2L, 2L)),
    rep(exception, 2L)
  )
  result <- check_law(new_law(
    "exception",
    list(x = only_exception),
    function(x) FALSE
  ))
  expect_identical(result@counterexample@minimal$x, exception)
  expect_identical(result@shrink_status, "complete")
}
only_finite <- gen_choice(
  gen_double(),
  gen_element(c(NA_real_, NaN, -Inf, Inf)),
  prob = c(1, 0)
)
expect_true(all(is.finite(gen_example(
  gen_vector(only_finite, 20L, 20L),
  size = 100L
))))
