...
expect_true(grepl(
  '"never": 0/10 (0%; minimum 100% unmet)',
  format_check_result(insufficient),
  fixed = TRUE
))
expect_true(grepl(
  "missed coverage requirements",
  format_check_result(insufficient),
  fixed = TRUE
))
...
expect_identical(
  check_law(unlabelled, tests = 3L)@status,
...
expect_identical(
  names(plain@coverage),
  c("label", "count", "proportion", "minimum", "met")
)
expect_identical(
  format_check_result(plain),
  "Law 'unlabelled' passed 3 tests (seed 1)."
)
...
local({
  classified <- 0L
  generator <- new_generator(function(size) 10L, function(x) {
    if (x == 10L) {
      list(5L)
    } else if (x == 5L) {
      list(0L)
...
never <- new_law(
  ...
  classify = function(x) "discarded",
  min_coverage = c(required = 0, absent = 1)
)
...
for (phase in c("draw", "classify", "holds")) {
  for (fault in c("error", "warning", "invalid")) {
    local({
      evaluated <- 0L
      defect <- function() {
        switch(
...
local({
  ...
  inner <- new_law(
    ...
    list(x = sizes),
    function(x) TRUE,
    classify = function(x) "inner",
    min_coverage = c(inner = 1)
  )
  outer <- new_law(
    "outer",
    list(x = sizes),
    ...
    classify = function(x) c("outer", "outer"),
    min_coverage = c(outer = 1)
  )
  ...
  expect_true(all(vapply(
    inner_results,
    function(x) identical(x@coverage$count, 2L),
    logical(1)
  )))
})
...
local({
  ...
  law <- new_law(
    "draws",
    list(x = new_generator(function(size) runif(1L))),
    function(x) {
      seen <<- c(seen, x)
      runif(1L)
      TRUE
...
for (minimum in list(
  ...
  c(x = 0.1, x = 0.2),
  structure(0.1, names = ""),
  structure(0.1, names = NA_character_),
  list(x = 0.1),
  matrix(0.1, 1L, 1L)
)) {
  expect_error(new_law(
    "invalid minimum",
    list(x = sizes),
    function(x) TRUE,
...
expect_error(
  new_law(
    ...
    list(x = sizes),
    function(x) TRUE,
    classify = function(y) "label"
  ),
  pattern = "`classify` must accept",
  fixed = TRUE
)
for (labels in list(
  1L,
