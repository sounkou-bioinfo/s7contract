  ...
  for (root in roots) {
    ...
    repeat {
      ...
      stopifnot(
        ...
        all(value %in% 1:4),
        length(changed) > 0L,
        value[changed[1L]] < root[changed[1L]]
      )
      checked <- checked + 1L
    }
  }
...
draws <- vapply(
  ...
  function(seed) {
    paste(
      gen_example(gen_sample(1:3, 2L), size = 0L, seed = seed),
      collapse = ""
    )
  },
  character(1)
...
expect_identical(
  gen_example(gen_sample(1:20, 4L), size = 0L),
...
for (source in sources) {
  for (k in unique(c(0L, length(source)))) {
    ...
    result <- check_law(new_law("sample", list(x = generator), function(x) {
      FALSE
    }))
    ...
    expect_identical(
      gen_example(gen_subsequence(source, k, k)),
      source[seq_len(k)]
    )
  }
}
...
for (minimum in 0:3) {
  local({
    ...
    law <- new_law(
      "ordered positions",
      list(x = gen_resize(gen_subsequence(1:8, minimum, 6L), 20L)),
      function(x) {
        stopifnot(
          length(x) >= minimum,
          length(x) <= 6L,
...
cases <- gen_bind(
  gen_vector(gen_integer(), min = 1L, max = 6L),
  function(values) {
    gen_product(
      values = gen_constant(values),
      i = gen_sample(seq_along(values))
    )
  }
)
law <- new_law(
  "dependent permutation",
  list(input = gen_resize(cases, 10L)),
  function(input) {
...
expect_identical(
  gen_example(gen_vector(gen_sample(1:3, 0L), 2L, 2L)),
...
local({
  calls <- 0L
  generator <- gen_map(large, function(x) {
    calls <<- calls + 1L
    x
  })
