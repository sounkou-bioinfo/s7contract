...
expect_identical(
  calendar_replayed@counterexample@minimal,
  calendar_failure@counterexample@minimal
)
expect_identical(
  calendar_replayed@shrink_attempts,
  calendar_failure@shrink_attempts
)
input <- calendar_failure@counterexample@minimal$input
expect_true(
  input$start == new_year && input$end == new_year && input$day == new_year
)

# Compare calendar meaning across storage modes; both include leap/year boundaries.
for (days in list(
  c("2023-12-31", "2024-01-01"),
  ...
  expect_identical(
    interval_contains(interval, integer_dates),
    interval_contains(interval, dates)
  )
  draws <- vapply(
    1:50,
    function(seed) {
      as.double(gen_example(
        date_generator(dates[1L], dates[length(dates)]),
        size = 10L,
        seed = seed
      ))
    },
...
for (size in 0:4) {
  dates <- vapply(
    1:20,
    function(seed) as.double(gen_example(generator, size, seed)),
    double(1)
  )
...
visits <- 0L
audited <- new_law(
  calendar_failure@law@name,
  calendar_failure@law@generators,
  function(input) {
    stopifnot(
      all(vapply(input[c("start", "end", "day")], whole_day, logical(1))),
      input$start >= first_day,
      input$end <= last_day,
      input$start <= input$end,
      input$x@start == input$start,
...
result <- do.call(
  check_law,
  c(list(law = audited), calendar_failure@parameters)
)
expect_identical(
...
expect_error(
  date_generator(first_day, last_day, last_day + 1L),
  pattern = "within"
)
expect_error(
  date_generator(first_day, last_day, first_day - 1L),
  pattern = "within"
)
expect_error(date_generator(
  .Date(-.Machine$integer.max),
  .Date(.Machine$integer.max)
))
