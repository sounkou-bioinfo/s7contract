...
dependent <- gen_bind(lengths, function(n) {
  gen_product(
    n = gen_constant(n),
    values = gen_vector(gen_resize(gen_integer(), 10L), n, n)
  )
})
seen <- list()
length_law <- new_law(
  "vectors have fewer than three elements",
  list(x = dependent),
  function(x) {
...
expect_identical(
  result@counterexample@minimal$x,
  list(n = 3L, values = integer(3L))
)
...
random_pairs <- gen_bind(lengths, function(n) {
  gen_product(
    n = gen_constant(n),
    random = new_generator(function(size) runif(1L))
  )
})
...
downstream <- gen_bind(gen_constant(NULL), function(x) {
  ...
  new_generator(function(size) 10L, function(x) {
    if (x > 0L) list(0L) else list()
  })
})
inner_result <- check_law(new_law(
  "inner shrink",
  list(x = downstream),
  function(x) FALSE
))
...
broken <- gen_bind(lengths, function(n) {
  if (n == 1L) {
    stop("dependent factory failed")
  }
  gen_constant(n)
})
broken_result <- check_law(new_law(
  "broken shrink",
  list(x = broken),
  function(x) FALSE
))
...
expect_identical(
  conditionMessage(broken_result@shrink_condition),
  "dependent factory failed"
)

# Choice shrinks across branches and then within the selected branch.
ten <- new_generator(
  function(size) 10L,
  function(x) if (x > 0L) list(0L) else list(),
  prototype = integer()
)
choices <- gen_choice(gen_constant(-1L), ten, prob = c(1, 1e12))
cross <- check_law(new_law("cross branch", list(x = choices), function(x) {
  FALSE
}))
expect_identical(cross@counterexample@original$x, 10L)
expect_identical(cross@counterexample@minimal$x, -1L)
within <- check_law(new_law("within branch", list(x = choices), function(x) {
  x < 0L
}))
...
excluded <- gen_choice(
  new_generator(function(size) stop("excluded")),
  ten,
  prob = c(0, 1)
)
excluded_result <- check_law(new_law(
  "excluded branch",
  list(x = excluded),
  function(x) FALSE
))
...
expect_identical(
  gen_example(gen_vector(choices, 2L, 2L)) |> typeof(),
  "integer"
)
...
expect_identical(
  gen_example(gen_element(c(NA_integer_, 1L), prob = c(1, 0))),
  NA_integer_
)
expect_identical(
  gen_example(gen_element(c("excluded", "chosen"), prob = c(0, 1))),
  "chosen"
)
expect_true(gen_example(gen_element(1:2, prob = c(1e308, 1e308))) %in% 1:2)
expect_identical(
  gen_example(gen_element(as.Date("2026-01-01"))),
  as.Date("2026-01-01")
)
letters_result <- check_law(new_law(
  "letters",
  list(x = gen_element(letters)),
  function(x) FALSE
))
...
size_value <- gen_sized(
  function(size) gen_constant(size),
  prototype = integer()
)
...
expect_identical(
  gen_example(
    gen_product(fixed = gen_resize(size_value, 7L), normal = size_value),
    size = 13L
  ),
  list(fixed = 7L, normal = 13L)
)
expect_identical(
  gen_example(gen_vector(size_value, 2L, 2L), size = 13L),
  c(13L, 13L)
)
...
expect_identical(
  gen_example(
    gen_recursive(gen_constant(0L), function(child) stop("expanded")),
    size = 0L
  ),
  0L
)
...
depth <- function(x) {
  if (is.list(x)) 1L + max(vapply(x, depth, integer(1))) else 0L
}
tree_values <- lapply(seq_len(30L), function(seed) {
  gen_example(tree_gen, size = 15L, seed = seed)
})
depths <- vapply(tree_values, depth, integer(1))
expect_true(all(depths <= 4L))
expect_true(any(depths > 1L))
expect_identical(
  lapply(tree_values, function(x) unlist(x, use.names = FALSE)) |>
    unlist(use.names = FALSE) |>
    unique(),
  0L
)
tree_result <- check_law(new_law(
  "tree",
  list(x = gen_resize(tree_gen, 15L)),
  function(x) FALSE
))
...
calls <- 0L
no_shrink_result <- check_law(new_law(
  "no shrink",
  list(x = no_shrink),
  function(x) FALSE
))
...
expect_identical(
  gen_example(gen_no_shrink(choices), seed = 2L),
  gen_example(choices, seed = 2L)
)
expect_identical(
  gen_example(gen_vector(gen_no_shrink(ten), 2L, 2L)),
  c(10L, 10L)
)
...
for (bad in list(gen_bind(ten, function(x) 1L), gen_sized(function(size) 1L))) {
  expect_error(
    gen_example(bad),
    pattern = "factory must return a generator",
    fixed = TRUE
  )
  failure <- check_law(new_law("invalid factory", list(x = bad), function(x) {
    TRUE
  }))
...
warning_result <- check_law(new_law("warning", list(x = warns), function(x) {
  TRUE
}))
...
local({
  ...
  expect_error(
    gen_example(gen_bind(ten, function(x) stop("failure"))),
    pattern = "failure"
  )
  expect_identical(.Random.seed, before)
  expect_identical(RNGkind(), kind_before)
