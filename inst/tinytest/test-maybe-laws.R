...
expect_identical(
  vapply(maybe_results, function(x) x@status, character(1)),
  c(
    left_identity = "passed",
    right_identity = "passed",
...
expect_true(maybe_equal(
  maybe_function(list(op = "add", amount = -2L))(3L),
  Just(value = 1L)
))
...
expect_error(
  maybe_function(list(op = "unknown"))(0L),
  pattern = "Unknown Maybe function"
)
local({
...
for (scenario in c("input_absent", "first_absent", "both_present")) {
  local({
    trace <- character()
...
expect_identical(
  vapply(defaulting_results, function(x) x@status, character(1)),
  c(
    left_identity = "falsified",
    right_identity = "falsified",
    associativity = "falsified"
  )
)
expect_identical(
  defaulting_results$left_identity@counterexample@minimal,
  list(value = 0L, fn = list(op = "nothing"))
)
expect_true(S7_inherits(
  defaulting_results$right_identity@counterexample@minimal$mx,
  Nothing
))
...
right <- dict_bind(DefaultingMaybe, example$mx, function(x) {
...
CoercingMaybe <- MonadDict(
  name = "Coercing Maybe",
  pure = function(value) Just(value = as.double(value)),
  bind = MaybeMonad@bind
)
coercing_results <- lapply(
  maybe_laws(CoercingMaybe),
  check_law,
  tests = 200L,
  seed = 1L
)
expect_identical(
  vapply(coercing_results, function(x) x@status, character(1)),
  c(
    left_identity = "falsified",
    right_identity = "falsified",
    associativity = "passed"
...
expect_true(isTRUE(expect_law(
  maybe_laws(MaybeMonad)$associativity,
  tests = 200L,
  seed = 1L
)))
