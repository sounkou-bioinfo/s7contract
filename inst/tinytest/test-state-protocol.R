...
expect_identical(
  vapply(
    store_failure@counterexample@minimal$sequence,
    function(step) step$command,
    character(1)
  ),
  c("put", "reset")
)
expect_identical(
  store_failure@counterexample@minimal$sequence[[1L]]$input,
  list(key = "a", value = 0L)
)
...
expect_true(grepl(
  "Step 2 'reset' (ensure)",
  format_check_result(store_failure),
  fixed = TRUE
))
...
visits <- 0L
audited <- new_law(
  store_failure@law@name,
  store_failure@law@generators,
  function(sequence) {
    keys <- character()
    ids <- integer()
    for (step in sequence) {
      ...
      switch(
        ...
        put = {
          stopifnot(
            grepl("^[abc\u00e9]{1,4}$", step$input$key),
            validUTF8(step$input$key)
          )
          keys <- union(keys, step$input$key)
        },
        get = {
          stopifnot(step$input %in% keys)
        },
        delete = {
          stopifnot(step$input %in% keys)
          keys <- setdiff(keys, step$input)
        },
        reset = {
          keys <- character()
        }
...
audited_result <- do.call(
  check_law,
  c(list(law = audited), store_failure@parameters)
)
expect_identical(
...
expect_identical(
  visits,
  audited_result@attempts + audited_result@shrink_attempts
)
...
expect_identical(
  truncated_failure@condition$input,
  list(key = "aa", value = 0L)
)
replayed <- do.call(
  check_law,
...
expect_identical(
  ...
  truncated_failure@counterexample@minimal
)
...
expect_identical(
  gen_example(gen_vector(string_generator("x", 0L, 0L), 2L, 2L)),
  c("", "")
)
...
for (minimum in 0:2) {
  ...
  expect_identical(
    result@counterexample@minimal$key,
