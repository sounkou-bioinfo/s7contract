...
for (results in vector_results) {
  ...
  expect_true(all(vapply(
    results,
    function(result) result@discards == 0L,
    logical(1)
  )))
}
...
expect_identical(
  ...
  failure@counterexample@minimal
)
...
for (arguments in list(
  failure@counterexample@original,
  failure@counterexample@minimal
)) {
...
for (make in implementations) {
  ...
  for (values in list(double(), c(-1, 0, 2), c(-0.25, 0, 0.5))) {
    ...
    if (length(values) > 0L) {
      indices <- c(indices, list(c(3L, 1L, 3L)))
    }
    for (i in indices) {
      input <- list(x = make(values), values = values, i = i)
      expect_true(all(vapply(
        laws,
        function(law) (law@holds)(input),
        logical(1)
      )))
    }
  }
...
audited <- new_law(failure@law@name, failure@law@generators, function(input) {
  stopifnot(
    all(input$i %in% seq_along(input$values)),
...
expect_identical(
  audit_result@counterexample@minimal,
  failure@counterexample@minimal
)
...
for (input in list(
  fractional_check@counterexample@original$input,
