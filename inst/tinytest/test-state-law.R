...
local({
  ...
  command <- new_command(
    ...
    generate = function(state) {
      new_generator(function(size) 10L, function(x) {
        if (x == 0L) list() else list(0L)
      })
    },
  ...
  expect_true(all(vapply(
    fixtures,
    function(fixture) fixture$closed,
    logical(1)
  )))
  expect_false(anyDuplicated(fixtures) > 0L)
  expect_identical(
    result@counterexample@original_condition$trace[[1L]]$before,
    0L
  )
...
local({
  commands <- list(
    ...
    new_command(
      "alias",
      generate = function(state) {
        if (state$stage == 2L) gen_constant(state$handle)
      },
      execute = function(fixture, input) {
    ...
    new_command(
      ...
      generate = function(state) {
        if (state$stage == 3L) {
          gen_product(
            ...
            value = new_generator(function(size) 10L, function(x) {
              if (x == 0L) list() else list(0L)
            })
          )
        }
      ...
      execute = function(fixture, input) {
        stopifnot(
          identical(input$nested$ref[[1L]], 12L),
  ...
  expect_identical(
    vapply(sequence, function(step) step$command, character(1)),
  ...
  audited <- new_law(law@name, law@generators, function(sequence) {
    ...
    for (step in sequence) {
      if (step$command == "alias") {
        stopifnot(step$input@id %in% ids)
      }
      if (step$command == "use") {
        stopifnot(step$input$nested$ref[[1L]]@id %in% ids)
      }
      ids <- c(ids, step$id)
    }
  ...
  audited_result <- do.call(
    check_law,
    c(list(law = audited), result@parameters)
  )
  expect_identical(
    audited_result@counterexample@minimal,
    result@counterexample@minimal
  )
  expect_true(any(lengths(visited) == 0L))
  expect_true(any(vapply(
    visited,
    function(x) length(x) == 1L && x[[1L]]$command == "allocate",
    logical(1)
  )))
})
...
local({
  ...
  commands <- list(
    ...
    new_command(
      "consume",
      generate = function(state) {
        if (state$stage == 1L) gen_constant(state$handle)
      },
      execute = function(fixture, input) {
  ...
  law <- new_state_law("NULL handles", list(stage = 0L), commands, function() {
    NULL
  })
  ...
  expect_identical(
    gen_example(gen_commands(list(stage = 0L), commands, max = 0L)),
    list()
  )
})
  ...
  local({
    ...
    command <- new_command(
      ...
      update = function(state, input, output) {
        if (phase == "update" && is.integer(output)) {
          stop("update failed")
        }
        state
      },
...
for (postcondition in c("false", "error")) {
  local({
    ...
    expect_identical(
      result@status,
      if (postcondition == "false") "falsified" else "error"
    )
    expect_identical(result@condition$phase, "ensure")
    expect_identical(
      conditionMessage(result@condition$cleanup_condition),
      "cleanup error"
    )
    expect_identical(
      result@counterexample@minimal,
      result@counterexample@original
    )
...
for (failure_phase in c("execute", "setup", "teardown")) {
  local({
    closed <- 0L
    made <- 0L
    fail_setup <- FALSE
    command <- new_command(
      ...
      execute = function(fixture, input) {
        fixture$input <- input
        if (input == 0L && failure_phase == "execute") {
          stop("later failure")
        }
        if (input == 5L && failure_phase == "setup") {
          fail_setup <<- TRUE
        }
        input
      },
    ...
    law <- new_state_law(
      ...
      teardown = function(fixture) {
        closed <<- closed + 1L
        if (identical(fixture$input, 0L) && failure_phase == "teardown") {
          stop("later failure")
        }
      }
    )
    ...
    expect_true(grepl(
      "later failure",
      conditionMessage(result@shrink_condition),
      fixed = TRUE
    ))
    expect_identical(closed, made)
  })
...
local({
  ...
  second <- new_command(
    "second",
    function(state) gen_constant(NULL),
    function(fixture, input) NULL,
    function(state, input, output) FALSE
  )
  law <- new_state_law(
    "same postcondition",
    NULL,
  ...
  original <- list(
    list(id = 1L, command = "first", input = 10L),
    list(id = 2L, command = "second", input = NULL)
  )
  law@generators <- list(
    sequence = new_generator(function(size) original, function(sequence) {
...
local({
  ...
  command <- new_command(
    "broken",
    generate = function(state) {
      new_generator(function(size) 1L, function(x) {
...
command <- new_command(
  ...
  function(state) gen_constant(NULL),
  function(fixture, input) NULL,
  function(state, input, output) TRUE
)
expect_error(
  new_command("", command@generate, command@execute, command@ensure),
  pattern = "one non-empty string",
  fixed = TRUE
)
expect_error(new_command("bad", NULL, command@execute, command@ensure))
expect_error(
  gen_commands(NULL, list()),
  pattern = "non-empty list",
  fixed = TRUE
)
expect_error(
  gen_commands(NULL, list(command, command)),
  pattern = "unique",
  fixed = TRUE
)
expect_error(
  new_state_law("bad", NULL, list(command), NULL),
  pattern = "must be functions",
  fixed = TRUE
)
expect_error(
  gen_commands(NULL, list(command), max = -1L),
...
for (fault in c("generate", "require", "ensure")) {
  broken <- command
  if (fault == "generate") {
    broken@generate <- function(state) 1L
  }
  if (fault == "require") {
    broken@require <- function(state, input) NA
  }
  if (fault == "ensure") {
    broken@ensure <- function(state, input, output) c(TRUE, FALSE)
  }
  law <- new_state_law("invalid callback", NULL, list(broken), function() NULL)
  result <- check_law(law, tests = 20L)
  expect_identical(result@status, "error")
  expect_true(grepl(
    if (fault == "generate") "generator or NULL" else "one non-missing logical",
...
result <- check_law(
  new_state_law("invalid generated input", NULL, list(broken), function() NULL),
  tests = 20L
)
expect_identical(result@status, "error")
expect_true(grepl(
...
local({
  ...
  make_law <- function(random) {
    command <- new_command(
      "draw",
      generate = function(state) {
        gen_bind(gen_integer(1L, 10L), function(n) gen_integer(0L, n))
      },
      execute = function(fixture, input) {
        if (random) {
          runif(9L)
