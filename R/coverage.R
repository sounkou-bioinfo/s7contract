.format_law_coverage <- function(result) {
  ...
  header <- sprintf(
    "Case coverage (%d accepted cases%s):",
    result@coverage_cases,
    partial
  )
  ...
  details <- vapply(
    ...
    function(i) {
      share <- if (is.na(coverage$proportion[[i]])) {
        ...
        paste0(
          format(100 * coverage$proportion[[i]], digits = 3, trim = TRUE),
          "%"
        )
      }
      required <- if (is.na(coverage$minimum[[i]])) {
        ""
      } else {
        paste0(
          "; minimum ",
          format(100 * coverage$minimum[[i]], trim = TRUE),
          "%",
      ...
      sprintf(
        "  %s: %d/%d (%s%s)",
        encodeString(coverage$label[[i]], quote = '"'),
        coverage$count[[i]],
        result@coverage_cases,
        share,
        required
  ...
  if (nrow(coverage) > length(rows)) {
    details <- c(
      details,
      sprintf(
        "  ... %d more labels in @coverage.",
        nrow(coverage) - length(rows)
      )
    )
  }
  c(header, details)
