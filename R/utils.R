...
.check_names <- function(names, arg) {
  if (is.null(names) || anyNA(names) || !all(nzchar(names)) || anyDuplicated(names)) {
...
.class_key <- function(class) {
  if (inherits(class, "S7_S3_class")) return(list(kind = "S3", name = class$class[[1L]]))
  if (isS4(class)) return(list(kind = "S4", package = class@package, name = class@className))
  if (inherits(class, "S7_union")) return(list(kind = "union", classes = lapply(class$classes, .class_key)))
  name <- if (is.null(class)) "NULL" else if (identical(class, S7::class_any)) {
    "ANY"
  } else if (identical(class, S7::class_missing)) "MISSING" else nameOfClass(class)
  list(kind = base::class(class)[[1L]], name = name)
}

.class_label <- function(class) {
  if (inherits(class, "S7_union")) {
    return(paste(vapply(class$classes, .class_label, character(1)), collapse = " | "))
  }
  key <- .class_key(class)
  name <- switch(key$kind,
...
.target_class_or_null <- function(x, arg = "x") {
  ...
  cls <- S7::S7_class(x)
  if (!is.null(cls)) return(cls)
  if (isS4(x)) return(S7::as_class(methods::getClass(class(x))))
  if (is.object(x)) return(S7::new_S3_class(class(x)))
  ...
  switch(
    ...
    closure = , special = , builtin = S7::class_function,
...
.normalise_requirements <- function(x, descriptor, constructor, arg) {
  if (is.null(x)) x <- list()
  if (is.function(x) || S7::S7_inherits(x, descriptor)) x <- list(x)
  if (!is.list(x)) .abort("`%s` must be a list of S7 generics or requirements.", arg)
  nms <- names(x)
  if (is.null(nms)) nms <- rep("", length(x))
  if (anyNA(nms)) .abort("`%s` names must be non-missing.", arg)
  for (i in seq_along(x)) {
    req <- x[[i]]
    if (!S7::S7_inherits(req, descriptor)) req <- constructor(req)
    if (nzchar(nms[[i]])) req@name <- nms[[i]]
    x[[i]] <- req
    nms[[i]] <- req@name
  }
  names(x) <- nms
...
.normalise_parents <- function(parents, descriptor, constructor) {
  if (is.null(parents)) return(list())
  if (S7::S7_inherits(parents, descriptor)) parents <- list(parents)
  if (!is.list(parents) || !all(vapply(parents, S7::S7_inherits, logical(1), descriptor))) {
